...
func TestFieldElementValidation(t *testing.T) {
	...
	zero := new(fe).zero()
	if !zero.isValid() {
		t.Fatalf("zero must be valid")
	}
	one := new(fe).one()
	if !one.isValid() {
		t.Fatalf("one must be valid")
	}
	if modulus.isValid() {
		t.Fatalf("modulus must be invalid")
	}
	n := modulus.big()
	n.Add(n, big.NewInt(1))
	if new(fe).setBig(n).isValid() {
		t.Fatalf("number greater than modulus must be invalid")
	}
}
...
func TestFieldElementHelpers(t *testing.T) {
	...
	if !odd.isOdd() {
		t.Fatalf("1 must be odd")
	}
	if odd.isEven() {
		t.Fatalf("1 must not be even")
	}
	even := new(fe).setBig(big.NewInt(2))
	if !even.isEven() {
		t.Fatalf("2 must be even")
	}
	if even.isOdd() {
		t.Fatalf("2 must not be odd")
	}
	// fe2
	zero2 := new(fe2).zero()
	if !zero2.isZero() {
		t.Fatalf("'zero' is not zero, 2")
	}
	one2 := new(fe2).one()
	if !one2.isOne() {
		t.Fatalf("'one' is not one, 2")
	}
	// fe6
	zero6 := new(fe6).zero()
	if !zero6.isZero() {
		t.Fatalf("'zero' is not zero, 6")
	}
	one6 := new(fe6).one()
	if !one6.isOne() {
		t.Fatalf("'one' is not one, 6")
	}
	// fe12
	zero12 := new(fe12).zero()
	if !zero12.isZero() {
		t.Fatalf("'zero' is not zero, 12")
	}
	one12 := new(fe12).one()
	if !one12.isOne() {
		t.Fatalf("'one' is not one, 12")
	}
}
