...
func TestFieldElementHelpers(t *testing.T) {
	...
	odd := new(fe).fromBig(big.NewInt(1))
	if !odd.isOdd() {
	...
	even := new(fe).fromBig(big.NewInt(2))
	if !even.isEven() {
...
func TestFieldElementSerialization(t *testing.T) {
	...
	t.Run("string", func(t *testing.T) {
		for i := 0; i < fuz; i++ {
			a, _ := new(fe).rand(rand.Reader)
			b, err := new(fe).fromString(a.string())
			if err != nil {
				t.Fatal(err)
			}
			if !a.equal(b) {
