...
func (e *fp6) mul(c, a, b *fe6) {
	...
	fp2Ladd(t[3], &a[1], &a[2]) // a1 + a2
	fp2Ladd(t[4], &b[1], &b[2]) // b1 + b2
	fp2.mulAssign(t[3], t[4])   // (a1 + a2)(b1 + b2)
	fp2Add(t[4], t[1], t[2])    // v1 + v2
	fp2SubAssign(t[3], t[4])    // (a1 + a2)(b1 + b2) - v1 - v2
	mulByNonResidueAssign(t[3]) // ((a1 + a2)(b1 + b2) - v1 - v2)β
	fp2AddAssign(t[3], t[0])    // c0 = ((a1 + a2)(b1 + b2) - v1 - v2)β + v0
	fp2Ladd(t[5], &a[0], &a[1]) // a0 + a1
	fp2Ladd(t[4], &b[0], &b[1]) // b0 + b1
	fp2.mulAssign(t[5], t[4])   // (a0 + a1)(b0 + b1)
	fp2Add(t[4], t[0], t[1])    // v0 + v1
	fp2SubAssign(t[5], t[4])    // (a0 + a1)(b0 + b1) - v0 - v1
	mulByNonResidue(t[4], t[2]) // βv2
	fp2Add(&c[1], t[5], t[4])   // c1 = (a0 + a1)(b0 + b1) - v0 - v1 + βv2
	fp2Ladd(t[5], &a[0], &a[2]) // a0 + a2
	fp2Ladd(t[4], &b[0], &b[2]) // b0 + b2
	fp2.mulAssign(t[5], t[4])   // (a0 + a2)(b0 + b2)
	fp2Add(t[4], t[0], t[2])    // v0 + v2
	fp2SubAssign(t[5], t[4])    // (a0 + a2)(b0 + b2) - v0 - v2
	fp2Add(&c[2], t[1], t[5])   // c2 = (a0 + a2)(b0 + b2) - v0 - v2 + v1
	c[0].set(t[3])
}
...
func (e *fp6) mulAssign(a, b *fe6) {
	...
	fp2Add(t[4], t[1], t[2])
	fp2SubAssign(t[3], t[4])
	mulByNonResidueAssign(t[3])
	fp2AddAssign(t[3], t[0])
	fp2Ladd(t[5], &a[0], &a[1])
	fp2Ladd(t[4], &b[0], &b[1])
	fp2.mulAssign(t[5], t[4])
	fp2Add(t[4], t[0], t[1])
	fp2SubAssign(t[5], t[4])
	mulByNonResidue(t[4], t[2])
	fp2Add(&a[1], t[5], t[4])
	fp2Ladd(t[5], &a[0], &a[2])
	fp2Ladd(t[4], &b[0], &b[2])
	fp2.mulAssign(t[5], t[4])
...
func (e *fp6) square(c, a *fe6) {
	...
	fp2DoubleAssign(t[1])       // s1 = 2a0a1
	fp2Sub(t[2], &a[0], &a[1])  // a0 - a1
	fp2AddAssign(t[2], &a[2])   // a0 - a1 + a2
	fp2.squareAssign(t[2])      // s2 = (a0 - a1 + a2)^2
	fp2.mul(t[3], &a[1], &a[2]) // a1a2
	fp2DoubleAssign(t[3])       // s3 = 2a1a2
	fp2.square(t[4], &a[2])     // s4 = a2^2
	mulByNonResidue(t[5], t[3]) // βs3
	fp2Add(&c[0], t[0], t[5])   // c0 = s0 + βs3
	mulByNonResidue(t[5], t[4]) // βs4
	fp2Add(&c[1], t[1], t[5])   // c1 = s1 + βs4
	fp2AddAssign(t[1], t[2])
	fp2AddAssign(t[1], t[3])
	fp2AddAssign(t[0], t[4])
	fp2Sub(&c[2], t[1], t[0]) // c2 = s1 + s2 - s0 - s4
}
...
func (e *fp6) mul01(c, a *fe6, b0, b1 *fe2) {
	...
	fp2SubAssign(t[2], t[1])    // b1(a1 + a2) - v1
	mulByNonResidueAssign(t[2]) // (b1(a1 + a2) - v1)β
	fp2Ladd(t[3], &a[0], &a[2]) // a0 + a2
	fp2.mulAssign(t[3], b0)     // b0(a0 + a2)
	fp2SubAssign(t[3], t[0])    // b0(a0 + a2) - v0
	fp2Add(&c[2], t[3], t[1])   // b0(a0 + a2) - v0 + v1
	fp2Ladd(t[4], b0, b1)       // (b0 + b1)
	fp2Ladd(t[3], &a[0], &a[1]) // (a0 + a1)
	fp2.mulAssign(t[4], t[3])   // (a0 + a1)(b0 + b1)
	fp2SubAssign(t[4], t[0])    // (a0 + a1)(b0 + b1) - v0
	fp2Sub(&c[1], t[4], t[1])   // (a0 + a1)(b0 + b1) - v0 - v1
	fp2Add(&c[0], t[2], t[0])   //  (b1(a1 + a2) - v1)β + v0
}
